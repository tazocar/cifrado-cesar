...
function cipher(){
	...
	for (var k = 0; k < textArr.length; k++) {
		...
		if (textArr[k] == " ") {
			textConvert.push(textArr[k]);
		//si no, cámbialo de letra a número.
		}else {

			textConvert.push(String.fromCharCode(textArr[k]))
		}
	}
...
function decipher(){
	...
	for (var k = 0; k < textEncryptedArr.length; k++) {
		//si el índice es = a espacio, mételo a textDecoded
		if (textEncryptedArr[k] == " ") {
			textDecoded.push(textEncryptedArr[k]);
		//si no, cambialo de número a letra
		} else {
			textDecoded.push(String.fromCharCode(textEncryptedArr[k]));
		}
	} 
